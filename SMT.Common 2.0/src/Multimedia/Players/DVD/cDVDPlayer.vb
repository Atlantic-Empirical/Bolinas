...
#Region "CONSTRUCTOR/DESTRUCTOR"
        ...
        Public Sub New(ByRef nParentForm As cSMTForm, ByVal nAVMode As eAVMode, ByVal nIntensityScaling As eScalingMode, ByVal nIntensitySignalResolution As eVideoResolution)

            If nAVMode <> eAVMode.DesktopVMR_NoKeystone Then
            ...
            Set(ByVal value As eRegions)
                Try
                    ...
                    Select Case AVMode
                        ...
                        Case eAVMode.DesktopVMR_NoKeystone
                            ...
                            HR = Graph.GraphBuilder.Connect(Graph.VidDec_SP_Out, Graph.VMR9_In_2)
                            If HR < 0 Then Marshal.ThrowExceptionForHR(HR)

                            HR = Graph.GraphBuilder.Render(Graph.VidDec_CC_Out) ', Graph.VMR9_In_3
                            If HR < 0 Then Marshal.ThrowExceptionForHR(HR)
...
#End Region 'API:PROPERTIES:USER CONFIG:PROC AMP
        ...
        Public Property NonSeamlessCellNotification() As Boolean
            ...
            End Set
        End Property
        Private _NSCNotification As Boolean = True
...
#Region "API:METHODS:PLAYER INITIALIZATION"
        ...
        Public Function InitializePlayer(ByVal VIDEO_TS As String, ByVal PlayerDefaults As sNavigatorSetup, Optional ByRef nViewerHostControl As cSMTForm = Nothing) As Boolean
            Try
                ...
                If Not Graph.KO_IKeystone Is Nothing Then Graph.KO_IKeystone.Pause(0)
                ...
                If Not RegionCompatibilityCheck() Then
                    If Graph.KO_IKeystone IsNot Nothing Then Graph.KO_IKeystone.Pause(1) 'we don't want them to see anything
...
#Region "API:METHODS:PLAY CONTROL:CORE"
        ...
        Public Overrides Function Play() As Boolean
            Try
                If (AVMode <> eAVMode.DesktopVMR) And (AVMode <> eAVMode.DesktopVMR_NoKeystone) Then
                ...
                ElseIf PlayState = ePlayState.Paused Then
                    ...
                    If AVMode = eAVMode.DesktopVMR_NoKeystone Then
                        HR = Graph.MediaCtrl.Run()
                        If HR < 0 Then Marshal.ThrowExceptionForHR(HR)
                    End If
...
ExecuteRW:
        ...
        Public Overrides Function FrameStep() As Boolean
            Try
                ...
                If AVMode = eAVMode.DesktopVMR Or AVMode = eAVMode.DesktopVMR_NoKeystone Then
...
#Region "FILTER GRAPH"
        ...
        Public Overrides Function BuildGraph(ByVal nFilePath As String, ByVal NotifyWindowHandle As Integer, ByVal nAVMode As eAVMode, ByVal nParentForm As Form) As Boolean
            Try
                Select Case AVMode
                    ...
                    Case eAVMode.DesktopVMR_NoKeystone
                        Return PhoenixGraph_Desktop_NoKeystone()
                End Select
            Catch ex As Exception
        ...
        Private Function PhoenixGraph_Desktop() As Boolean
            Try
                ...
                'OLD WORKS
                HR = Graph.GraphBuilder.Connect(Graph.VidDec_CC_Out, Graph.VMR9_In_2)
                If HR < 0 Then Marshal.ThrowExceptionForHR(HR)
                ...
                Return True
            Catch ex As Exception
                RaiseEvent evConsoleLine(eConsoleItemType.ERROR, "Problem with PhoenixGraph_Desktop(). Error: " & ex.Message, Nothing, Nothing)
            End Try
        End Function

        Private Function PhoenixGraph_Desktop_NoKeystone() As Boolean
            Try
                If Graph Is Nothing Then Graph = New cSMTGraph(Me.ParentForm.Handle)
                ...
                HR = Graph.GraphBuilder.Connect(Graph.VidDec_CC_Out, Graph.VMR9_In_3)
                If HR < 0 Then Marshal.ThrowExceptionForHR(HR)
                ...
                Return True
            Catch ex As Exception
                RaiseEvent evConsoleLine(eConsoleItemType.ERROR, "Problem with PhoenixGraph_Desktop_NoKeystone(). Error: " & ex.Message, Nothing, Nothing)
            End Try
        End Function
...
#Region "Video"
        ...
        Private Sub VideoFrameReceivedTimer_Tick(ByVal sender As Object, ByVal e As EventArgs) Handles VideoFrameReceived_Timer.Tick
            Try
                If Me.AVMode = eAVMode.DesktopVMR Or AVMode = eAVMode.DesktopVMR_NoKeystone Then Exit Sub
